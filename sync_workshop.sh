...
description=
description=[b]Survival fail-safe:[/b] eats, drinks, sleeps, bandages, and [b]flees[/b] when zombies actually threaten - wanderers outside your walls are ignored. It never fights: Build 42 gives an AI-driven character no way to swing a weapon, so the mod runs instead of standing its ground. Keeps a small stockpile with short near-home loot trips.
description=
