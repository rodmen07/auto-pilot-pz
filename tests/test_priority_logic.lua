...
-- Real Utils, because AutoPilot_Needs calls into it on the paths under test.
-- (The V4.5 detector/ownership cases that used to be the stated reason left in
-- slice 6; square iteration is a no-op here -- getCell() returns a stub whose
-- squares are all nil.)
...
-- ── Where the moved blocks went ──────────────────────────────────────────────
-- Six verbatim-move code-health slices have taken blocks out of this file.  A
-- helper kit leaves with the block when that block is its last caller here, so
-- each destination suite's own header carries that slice's measurement:
--
--   2. V5.4 endurance recovery (V5.4-1..17)  -> tests/test_rest_recovery.lua
--   3. V5.8 seat and status (V5.8-1..7)      -> tests/test_status_honesty.lua
--      (kit: drivenPlayer, repAt)
--   4. the un-versioned tail: the sleep pain-gate, the unhappy-relief dead
--      clause and the rest seating-data preference, all three 2026-07-24
--      in-game smoke-test regressions -> tests/test_smoke_regressions.lua
--      (kit: noFurniture, resetRest, mockFurniture, placeFurniture, windedPlayer)
--   5. V5.2 auto-day rotation (Tests 44-47)  -> tests/test_auto_rotation.lua
--      (kit: autoExercisePlayer, fatigueAndAdvance; FULL_SET_MS was COPIED, not
--      moved, the V3.2 / V4.5 / V4.6 blocks still using it at the time)
--   6. the exercise TRAINER family: V3.2 XP-fatigue detection, V4.5
--      intervention backoff / mod-action ownership, V4.6 the opt-in daily cap
--      (Tests 24-41) -> tests/test_exercise_trainer.lua (kit: exercisePlayer,
--      productiveSet, FULL_SET_MS, and the EXERCISE_DAILY_CAP = 0 pin).  The
--      three blocks were not separable: V4.5 and V4.6 both call V3.2's
--      exercisePlayer.  count_action_type STAYED -- it is defined under the
--      V4.7 header below that cut and every caller is below it too.
--
-- Gone with slice 3: the explicit MockTime.advance(EXERCISE_FATIGUE_RECOVERY_MS)
-- that slice 2 added right here.  It existed only so the V5.8 cases could train
-- despite the XP fatigue the V5.2 auto-rotation block left on the fitness pool.
-- Slices 5 and 6 have now taken every training block out of this file, so
-- nothing below the boredom-preference cases trains at all.
