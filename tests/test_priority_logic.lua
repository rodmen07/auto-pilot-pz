...
-- The V5.4 endurance-recovery cases (V5.4-1..17) moved to
-- tests/test_rest_recovery.lua (code-health split, 2026-08-07, second slice)
-- and the V5.8 seat-and-status cases (V5.8-1..7) to
-- tests/test_status_honesty.lua (third slice, same day).
--
-- The FOURTH slice took the un-versioned tail -- the sleep pain-gate, the
-- unhappy-relief dead clause and the rest seating-data preference, all three
-- regressions from the 2026-07-24 in-game smoke test -- to
-- tests/test_smoke_regressions.lua.  The furniture / endurance helper kit
-- (noFurniture, resetRest, mockFurniture, placeFurniture, windedPlayer) went
-- WITH it rather than staying: that tail was its last caller in this file.
-- drivenPlayer/repAt had already left with the V5.8 block for the same reason.
--
-- The FIFTH slice (2026-08-08) took the V5.2 auto-day rotation cases (Tests
-- 44-47) to tests/test_auto_rotation.lua, and their helper kit
-- (autoExercisePlayer, fatigueAndAdvance) went with them: both were defined
-- inside that block and called only inside it.  FULL_SET_MS was COPIED rather
-- than moved -- it is a derived constant and the V3.2 / V4.5 / V4.6 blocks
-- above still use it.
--
...
-- Slice 5 has now taken that block out of this file too, so nothing below the
-- V4.6 XP-cap cases trains at all and there is no fatigue left to wait out.
