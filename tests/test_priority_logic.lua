...
-- The V5.4 cases themselves (V5.4-1..17) moved to tests/test_rest_recovery.lua
-- (code-health split, 2026-08-07, second slice) and the V5.8 seat-and-status
-- cases (V5.8-1..7) to tests/test_status_honesty.lua (third slice, same day).
-- The furniture kit above STAYS: the un-versioned tail below still calls it
-- (resetRest, placeFurniture, windedPlayer, noFurniture).
--
-- drivenPlayer/repAt went WITH the V5.8 block: those two V5.7-era helpers had
-- no other caller left in this file once the block moved.
--
-- Gone with it: the explicit MockTime.advance(EXERCISE_FATIGUE_RECOVERY_MS)
-- that slice 2 added right here.  It existed only so the V5.8 cases could
-- train despite the XP fatigue the V5.2 auto-rotation block leaves on the
-- fitness pool; nothing in the tail below trains, so there is no fatigue left
-- to wait out.
