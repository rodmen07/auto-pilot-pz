...
- 19 Lua modules under `42/media/lua/client/` (`AutoPilot_Barricade.lua` removed in V5.0, `AutoPilot_SessionHistory.lua` added in V4.2, `AutoPilot_Consumption.lua` and `AutoPilot_Sleep.lua` split out of `AutoPilot_Needs` in a 2026-07-20 code-health pass — eat/drink then sleep, both verbatim moves); 14 Lua test suites, 1107 assertions, 0 failures (verified 2026-07-20); luacheck clean across 19 files; Python tooling suite green aside from two long-known-stale console-log assertions excluded from CI (tracked in the AutoPilot backlog `## Bugs`).
