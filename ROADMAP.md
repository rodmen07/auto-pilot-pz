...
- Non-exercise leveler skills (Tailoring, Mechanics, Cooking, Fishing, Foraging): 42.19 offers no clean queueable action path for most of them, which is why they were never scheduled directly. **Partially superseded 2026-07-21** by the Skills/Foraging scope reopening above — but the underlying constraint is unchanged and is exactly what the V7.0 proposal must establish per skill: a real, verified, queueable action path, or the skill stays out. Where a queued-action path already exists (skill-book reading), the V4.0 expansion proposal remains the precedent. The carpentry-via-barricade-pass route stays closed: V5.0 removed barricading from the mod's scope entirely and that non-goal was NOT reopened.
