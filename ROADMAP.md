...
- 22 Lua modules under `42/media/lua/client/` (`AutoPilot_Media.lua` added 2026-07-25 for tv/radio boredom relief, `AutoPilot_Barricade.lua` removed in V5.0, `AutoPilot_SessionHistory.lua` added in V4.2, `AutoPilot_Consumption.lua`/`AutoPilot_Sleep.lua`/`AutoPilot_Rest.lua`/`AutoPilot_Exercise.lua` split out of `AutoPilot_Needs` in a 2026-07-20 code-health pass — eat/drink, then sleep, then endurance-critical rest, then the exercise/trainer block, all verbatim moves; the rest move needed a same-day prior seam increment first since its cooldown state was shared with `check()`'s own gate, the exercise move did not since its shared touch points were already named functions); 19 Lua test suites, 1238 assertions, 0 failures (re-measured live 2026-07-25 after the media-relief PR; was 1197 after PR #80, 1176 after PR #78, 1158 after PR #77, 1137 after PR #76 and 1107 at V5.8, and the intermediate 1131 figure predated PR #74's flee-only rewrite of the threat/combat suites, which replaced fight cases with flee cases — re-measure this figure, never carry it forward); luacheck clean across 22 files; Python tooling suite green in CI (one run-log-dependent test, `TestRunLogSuspiciousPatterns`, is `@skipUnless(RUN_LOG.exists())` — it is skipped in CI and only runs, and can go red, when a real local run log is present; the old console-log assertions this line used to flag were fixed in PR #49). `AutoPilot_Needs.lua` down to 829 lines from 1848 (706 immediately after the four slices; +79 in PR #77's mood-relief seam and literacy fix, +18 in PR #78's scope note, +7 in the mood-food-choice gate comment, +20 in the media arm and its comment) — still well under the 1000-line code-health threshold. `AutoPilot_Inventory.lua` is 1061 lines, over that threshold but ruled OUT as a split target on commit share (10 percent) in a 2026-07-20 product increment.
