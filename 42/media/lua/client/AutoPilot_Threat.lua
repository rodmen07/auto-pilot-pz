...
local function doFight(player, zombies)
    ...
    local shouldEquip = weapon and (not primary)
    if not shouldEquip and weapon and weaponDamage then
...
-- Force fight regardless of moodle state.
...
-- Force flee regardless of moodle state.
...
function AutoPilot_Threat.check(player)
    ...
    if not weapon and #zombies > 1 then
        print("[Threat] FLEE — unarmed and " .. #zombies .. " zombies.")
        if not doFlee(player, zombies) then doFight(player, zombies) end
        return true
    end
