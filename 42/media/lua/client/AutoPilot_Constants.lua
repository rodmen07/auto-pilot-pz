...
-- Tight bandage-loot radius.  Intentionally small so the character does not
-- wander far while actively bleeding; nearby first-aid kits are almost always
-- in the same room.
AutoPilot_Constants.MEDICAL_LOOT_RADIUS = 8

-- Radius for placing an inventory item into the nearest container.
AutoPilot_Constants.PLACE_SEARCH_DIST = 20

-- Radius for rest-furniture search (beds > sofas > chairs).
AutoPilot_Constants.REST_SEARCH_DIST = 30

-- General container-loot radius used for food, drink, and readable items.
-- 80 tiles covers a typical house + yard without crossing far into unexplored areas.
...
-- Outdoor-square search for boredom relief.  Larger than loot radius because
-- outdoor tiles may be far away in dense urban environments.
AutoPilot_Constants.OUTDOOR_SEARCH_DIST = 120

-- ── Home / safehouse ──────────────────────────────────────────────────────────

-- Default containment circle radius when AutoPilot is first enabled.
-- 15 tiles fits comfortably inside a typical two-storey suburban house.
AutoPilot_Constants.HOME_DEFAULT_RADIUS = 15

-- Wide bed-search parameters when no home circle is set.
-- 100 tiles covers a city block; 3 floors = ground + one up + one down.
AutoPilot_Constants.BED_SEARCH_DIST   = 100
...
AutoPilot_Constants.EXERCISE_EQUIP_SEARCH_RADIUS = 30   -- tiles
