...
The runtime is 20 modules under `42/media/lua/client/` (`AutoPilot_Consumption`,
`AutoPilot_Sleep`, and `AutoPilot_Rest` all added 2026-07-20, split out of
`AutoPilot_Needs` in a code-health pass — see their rows below). PZ loads
...
| Module | Responsibility | Key interactions |
|---|---|---|
| `AutoPilot_Needs` | Priority state machine for survival needs plus the exercise slot: shelter, clothing, wound treatment dispatch, boredom relief, proactive scavenging, and `doExercise` (endurance gates, optional daily set cap (V4.6: 0 = unlimited by default), equipment preference, XP-fatigue rotation, and the V4.5 player-intervention backoff: a mod-queued set that vanishes uncompleted without a mod-initiated clear, an observed manual exercise, or an F10 panic stop holds training off for `EXERCISE_BACKOFF_MINUTES` game minutes instead of re-queuing over the player). Eat/drink moved to `AutoPilot_Consumption`, sleep to `AutoPilot_Sleep`, endurance-critical rest-in-place to `AutoPilot_Rest` (all 2026-07-20); `check`'s hunger/thirst/fatigue/exhausted branches and `forceEat`/`forceDrink`/`forceSleep`/`forceRest`/`seatPriorityForSprite` now delegate there, and `getEmptyLootCycles` delegates to `Consumption.getEmptyLootCycles`. `AutoPilot_Needs.setActivity` is the cross-module accessor `AutoPilot_Rest.doRest` uses to write the shared activity string, since it no longer lives in this file. | Called by `Main` (`check`, `shouldInterrupt`, and the V4.5 notifications `noteForeignExercise`/`noteModExerciseCleared`/`notePanicStop`). Calls `Medical.check`/`hasCriticalWound`, `Consumption.doEat`/`doDrink`, `Sleep.doSleep`, `Rest.doRest`/`isRestHoldActive`/`clearRestHold`, many `Inventory` helpers, `Home` bounds checks, `Leveler.check`, and `Telemetry.setDecision` for every decision. Exposes `trainExercise` (the Leveler's seam) and `getExerciseStatus` (the panel's status line). |
...
1. **luacheck**: zero errors and zero warnings across the 20 modules in
   `42/media/lua/client/`.
