...
The runtime is 22 modules under `42/media/lua/client/` (`AutoPilot_Consumption`,
...
rows below; `AutoPilot_Media` added 2026-07-25 as a NEW behaviour, not a
split). PZ loads
client Lua files alphabetically; `AutoPilot_Constants` loads before every
module that reads it at load time, and the module that sorts earlier
(`Adaptive`) only touches Constants inside function bodies, never at load.
...
1. **luacheck**: zero errors and zero warnings across the 22 modules in
   `42/media/lua/client/`.
